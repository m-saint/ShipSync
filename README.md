...
ShipSync remembers your ship's hull, crew, mettle, fires, officers, supplies, conditions, scene weather, and a chronological Captain's Log of every edit you've made. It does not roll dice, advance phases for you, or tell you what the rules say — your rulebook still does that. ShipSync just keeps the bookkeeping out of your way.

## Status

v1.0.4 — second post-deploy usability pass. v1.0 lands the readiness + preferences slates; v1.0.1 closed the v1.0 UX-audit deferrals; v1.0.2 corrected rule-fidelity drift; v1.0.3 closed nine usability defects a real session surfaced; v1.0.4 closes a second batch of urgent UX bugs and three quality features the same captain's session surfaced.

What changed in v1.0.4:

- **The page never reveals a "void" beneath the dashboard.** `tokens.css` pins `<body>` to `position: fixed; inset: 0` on the dashboard route (gated by a `data-route='dashboard'` attribute set in `App.svelte`). The previous `overflow: hidden` on html+body wasn't enough — a trackpad rubber-band could still pull the document up to reveal a blank surface beneath the rails. With `<body>` fixed to the viewport, wheel events from the side rails or the central scroll never chain past the body. Print routes opt out via the `data-route='print'` attribute so PrintView and FleetPrintView stay tall and scrollable for review before Cmd+P.
- **"Player character" is now synonymous with "captain."** The Charter dialog dropped its full PC card (name + portrait + stats grid) for a single optional Captain field that seeds the captain station. ShipDetail's heading reads `Captained by <captain>` and links the user straight to the captain card instead of rendering a separate PC section. Old saves migrate automatically — `migrateLegacyPlayerCharacter` (in `loadFile.js`, also called from `bundleFile.js` and `hydrateFromSnapshot`) folds any pre-v1.0.4 `playerCharacter.name` and portrait onto `officers.captain` if the captain station is empty, then drops the legacy field. The `PlayerCharacterSection.svelte` component was deleted.
- **Stores Below Decks steppers are symmetric.** `NumberStepper.svelte` gained a `fullWidth` prop. `SuppliesSection.svelte` opts in so the Grub / Grog / Gear steppers stretch to fill their grid cell, with `flex-1` on the input pinning the number dead-center and the `−` and `+` buttons to identical 32px ends. The pre-v1.0.4 layout grew the rightmost segment because the stepper was a flex row with a fixed-width input — the captain saw a `+` that read like a wider button than the `−`.
- **Each weapon slot can now name its actual mounts.** `Ship.weaponInventory` is a new schema field — a record of `{ bow, port, starboard, stern }`, each side a list of `{ id, name, slotsOccupied }` mounts. `WeaponMountList.svelte` renders the per-side list with a name input, a slots-occupied stepper, a remove button, and an "Add mount" button below. The slot-count steppers stay editable (they're a captain-declared *capacity*); a small "X of Y slots filled" tally appears beneath each side, amber if the mounts overflow capacity. Migration in `loadFile.js` / `bundleFile.js` / `hydrateFromSnapshot` adds an empty `weaponInventory` to old ships without touching their existing slot counts.
- **Side rails collapse independently.** `LoadedShipsRail.svelte` and `ScenePanel.svelte` each accept an `oncollapse` callback and render a chevron-only collapse button in their header. `Dashboard.svelte` owns two `$state` flags (`fleetRailCollapsed`, `sceneRailCollapsed`) that swap each rail for a 36px-wide vertical-text spine button labeled FLEET or SCENE & LOG. Per the v1.0.4 product call (`rails_persist: session_only`), the collapsed state is *not* persisted — every fresh tab boots both rails open.
- **Activity Log entries coalesce on the same field.** The `commit()` function in `workspace.svelte.js` already coalesced same-resource edits during a single uninterrupted edit run for combat resources (HP, Mettle, Fires); v1.0.4 extends the `coalesceKey` mechanism to every per-ship stepper field — supplies (Grub / Grog / Gear), hull max, explosion DC, speed (squares + knots), per-side weapon slot capacity, individual weapon mount slot counts, crew max + skeleton, scene round, pursuit gap + escape timer, scene-ship hp + fires + explosion DC, single-axis flag reputation. A burst of "gear 1 → 2 → 3 → 4 → 5" now collapses into a single "gear 1 → 5" entry, and the chain breaks naturally on a different field, a different ship, or any unrelated commit. Undo still walks one entry per logical change instead of one per click, so a fast stepper run unwinds in one step.
- **The "Captain's Log" name is back where it belongs.** The per-ship narrative session journal (`sessionHistory: SessionEntry[]` plus the `JournalSection.svelte` and `PastSessionEditor.svelte` editors) is retired — the v1.0.4 session decided that auto-generated session recaps were out of scope. The "Captain's Log" name is reclaimed for the workspace-level edit log in the right rail (formerly "Activity Log"). Each ship now carries a single `lastModifiedAt: string|null` timestamp instead of a session journal, and `isShipDirty()` reads off it. Migration is in place in three places (`loadFile.js`, `bundleFile.js`, and `workspace.svelte.js`'s `hydrateFromSnapshot`): the latest action timestamp from any legacy `sessionHistory` is folded into `lastModifiedAt`, then `sessionHistory` is dropped. The `'journal'` action category and its UI affordances were removed; old `localStorage` filter preferences pointing at `'journal'` are coerced to `'all'` on read.
